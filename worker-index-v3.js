...
async function applyPostcode(page, cap) {
  ...
  if (applied) {
    ...
    const suggestionClicked = await page.evaluate(() => {
      ...
      const selectors = [
        "[role=option]", "[role=listbox] li", "li[class*=suggest]", "div[class*=suggest] li",
        "li[class*=autocomplete]", "div[class*=autocomplete] li", "ul[class*=result] li"
      ];
      const option = [...document.querySelectorAll(selectors.join(","))]
        .find(el => visible(el) && (el.textContent || "").trim().length > 1);
      option?.click();
